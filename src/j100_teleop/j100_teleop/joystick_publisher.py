#!/usr/bin/env python3
"""
PyQt5 기반 가상 조이스틱 UI — geometry_msgs/msg/Twist 퍼블리셔 (Node 1: Pub 전용)

[이 파일이 하는 일]
  마우스로 드래그하는 2D 조이스틱 위젯을 화면에 띄우고,
  조이스틱 위치를 선속도(linear.x)·각속도(angular.z)로 변환하여
  중간 토픽 /joystick_cmd 로 20Hz(50 ms 주기)마다 발행합니다.

[왜 노드를 두 개로 쪼갰나? — 모놀리식(joystick_ui.py) → 2-노드 분리]
  기존 joystick_ui.py 는 "UI + 발행 + (가상의) 워치독/안전로직" 을 한 노드에
  몰아넣은 단일 노드였습니다. 이를 다음 두 노드로 분리합니다.

    ┌────────────────────┐   /joystick_cmd    ┌──────────────────┐  /j100_0001/cmd_vel  ┌──────────┐
    │ joystick_publisher │  ───────────────▶  │  cmd_vel_relay   │ ───────────────────▶ │   J100   │
    │  (이 파일, Node 1)  │  geometry_msgs/    │   (Node 2)       │  geometry_msgs/      │ 로봇/시뮬 │
  ...
  분리의 이점(교육 목적 핵심):
    1) 관심사 분리(SoC): UI/입력 노드와 안전/중계 노드를 독립적으로 테스트·교체 가능.
    2) 책임 분리: 워치독·타임아웃·안전 0 발행 같은 "로봇 보호 로직"은 Node 2 로 이동.
       Node 1 은 사람이 만든 입력을 충실히 발행만 합니다.
    3) 실행기(Executor) 모델 차이를 보여주는 좋은 예제:
       - 이 노드(Node 1)는 ROS 콜백이 0개 → SingleThreadedExecutor 로 충분.
       - cmd_vel_relay(Node 2)는 구독 콜백 + 워치독 타이머가 동시에 돌아야 하므로
         MultiThreadedExecutor 가 필요. (자세한 이유는 main() 주석 참고)

[모놀리식 대비 이 노드에서 바뀐 점]
  · 발행 토픽: /j100_0001/cmd_vel → /joystick_cmd (로봇에 직접 쏘지 않고 Node 2 경유)
  · 워치독/타임아웃에 "내가 끊기면 로봇 정지" 같은 안전 로직은 제거됨(이제 Node 2 책임).
    단, "끊김 없는 20Hz 출력" 자체는 여전히 이 노드의 책임입니다(아래 QTimer 주석 참고).
  · 노드 이름: joystick_ui → joystick_publisher

[ROS2 노드 정보]
  노드 이름 : joystick_publisher
              → 실행 후 `ros2 node list` 를 치면 /joystick_publisher 가 보입니다.
  발행 토픽 : /joystick_cmd        (중간 토픽, Node 2 cmd_vel_relay 가 구독)
  메시지 타입: geometry_msgs/msg/Twist

[실행 방법]
  이 파일은 j100_teleop 패키지의 발행(입력) 노드입니다.
  $ source /opt/ros/humble/setup.bash        # ROS2 환경 설정 (한 번만)
  $ source install/setup.bash                # 패키지 빌드 후 오버레이 설정
  $ ros2 run j100_teleop joystick_publisher  # Node 1 단독 실행
  $ ros2 run j100_teleop cmd_vel_relay       # Node 2 도 함께 실행해야 로봇이 움직임

[통신 대상]
  이 노드의 직접 통신 대상은 로봇이 아니라 Node 2(cmd_vel_relay) 입니다.
  cmd_vel_relay 가 /joystick_cmd 를 구독해 워치독을 적용한 뒤
  /j100_0001/cmd_vel 로 중계해야 Clearpath J100(Jackal) 또는 그 시뮬레이터가 움직입니다.
"""
# ── 표준 라이브러리 ────────────────────────────────────────────────────────
...
from geometry_msgs.msg import Twist             # /joystick_cmd 에 쓰이는 메시지 타입
...
# 발행할 ROS2 토픽 이름 — 모놀리식에서 가장 크게 바뀐 부분.
#   기존: '/j100_0001/cmd_vel' (로봇에 직접 발행)
#   변경: '/joystick_cmd'      (중간 토픽. Node 2 cmd_vel_relay 가 구독해서
#                               워치독을 적용한 뒤 /j100_0001/cmd_vel 로 중계함)
#   앞에 '/'가 붙으면 절대 경로이므로 어떤 네임스페이스에서 실행해도
#   항상 이 이름 그대로 발행됩니다. (네임스페이스 의존성 제거 의도)
#   확인: `ros2 topic list | grep joystick_cmd`
JOY_CMD_TOPIC = '/joystick_cmd'

# 발행 주기: 50 ms = 20 Hz.
#   모놀리식에서는 "로봇 watchdog(twist_mux 0.5 s) 에 안 걸리려고" 자주 보냈습니다.
#   2-노드 구조에서는 그 워치독 책임이 Node 2(cmd_vel_relay)로 옮겨갔지만,
#   그래도 이 노드는 여전히 끊김 없는 20 Hz 스트림을 내보내야 합니다. 이유:
#     · Node 2 의 워치독도 "최근에 입력이 들어왔는가"를 보고 판단하므로,
#       Node 1 이 일정 주기로 보내 줘야 Node 2 가 살아있다고 판단합니다.
#     · 일정 주기 발행이라야 가속/정지가 부드럽고 지연이 작습니다.
#   일반적으로 cmd_vel 계열은 10 ~ 50 Hz 가 적합합니다.
#   너무 느리면 Node 2 워치독에 걸려 정지, 너무 빠르면 CPU/네트워크 낭비.
PUBLISH_PERIOD_MS = 50          # 20 Hz, Node 2 워치독 타임아웃보다 충분히 빠름
...
# Joystick widget
# ────────────────────────────────────────────────────────────────────────
# [재사용 메모] 아래 JoystickWidget 클래스는 순수 Qt 코드로 ROS 의존성이 전혀
# 없으므로 모놀리식(joystick_ui.py)에서 거의 그대로 가져왔습니다. 2-노드 분리는
# UI 동작(UX)을 바꾸지 않으므로 위젯 코드/주석을 유지하는 것이 옳습니다.
...
# ────────────────────────────────────────────────────────────────────────
# ROS2 node (Node 1: /joystick_cmd 퍼블리셔 — Pub 전용)
# ────────────────────────────────────────────────────────────────────────
class JoystickPublisherNode(Node):
    """/joystick_cmd 퍼블리셔를 담당하는 ROS2 노드 클래스 (Node 1).

    rclpy.node.Node 를 상속받아 ROS2 노드로서의 기능(퍼블리셔 생성,
    로깅 등)을 사용합니다.

    [이 노드는 'Pub 전용' 입니다]
      · 서브스크라이버 없음, rclpy 타이머 없음, 어떤 ROS 콜백도 없음.
      · 모놀리식과 달리 워치독/안전 로직을 가지지 않습니다(그건 Node 2 책임).
      · 따라서 ROS 측에서 처리해야 할 콜백 이벤트가 0개입니다.
        → main() 에서 SingleThreadedExecutor 로 충분한 이유의 핵심.
    """

    def __init__(self):
        # ── ROS2 노드 초기화 ──────────────────────────────────────────────
        # super().__init__('joystick_publisher') 는 부모 클래스 Node 의 __init__ 호출.
        # 인자 문자열 'joystick_publisher' 가 이 노드의 ROS2 그래프상 이름이 됩니다.
        # 실행 후 `ros2 node list` 를 입력하면 /joystick_publisher 로 나타납니다.
        # (모놀리식에서는 'joystick_ui' 였습니다 — 역할이 바뀌었으므로 이름도 변경.)
        ...
        # 모놀리식 파일과 '완전히 동일한' 설정을 사용합니다(아래 값 변경 금지).
        #
        # ★★★ #1 함정(GOTCHA) ★★★
        #   QoS 는 퍼블리셔와 서브스크라이버가 '호환(compatible)' 되어야만
        #   메시지가 흐릅니다. 이 퍼블리셔가 BEST_EFFORT 이므로,
        #   Node 2(cmd_vel_relay)의 /joystick_cmd 구독자도 반드시
        #   BEST_EFFORT(호환 가능한) QoS 로 만들어야 합니다.
        #   만약 Node 2 구독자가 RELIABLE 로 설정되면 QoS 비호환이 되어
        #   "토픽 이름은 맞는데 메시지가 단 한 개도 안 들어오는" 현상이 발생합니다.
        #   (RELIABLE 구독자 + BEST_EFFORT 퍼블리셔 = incompatible)
        #   증상 진단: `ros2 topic echo /joystick_cmd` 는 보이는데
        #             Node 2 만 못 받는다면 100% QoS 불일치를 의심하세요.
        qos = QoSProfile(
            ...
            depth=10,
            # depth(큐 크기)=10: 구독자(Node 2)가 처리 속도가 느릴 때 최대 10개까지
        ...
        #    (단, 그 클램핑은 이 노드가 아니라 Node 2 이후 단계에서 적용됩니다.)
        #
        # ② 두 번째 인자 — 토픽 이름: JOY_CMD_TOPIC = '/joystick_cmd'
        #    모놀리식과 달리 로봇의 cmd_vel 이 아니라 '중간 토픽' 으로 발행합니다.
        #    앞에 '/' 가 붙으면 절대 경로 토픽이므로, 이 노드가 어느 네임스페이스에서
        #    실행되더라도 항상 '/joystick_cmd' 로 발행됩니다.
        #    이 토픽은 Node 2(cmd_vel_relay) 가 구독한 뒤 워치독을 적용하고
        #    /j100_0001/cmd_vel 로 중계합니다.
        #    확인: `ros2 topic list | grep joystick_cmd`
        #
        # ③ 세 번째 인자 — QoS 설정 객체 (위에서 정의한 qos 변수)
        #    정수(depth만)도 가능하지만, 여기서는 QoSProfile 로 상세 설정합니다.
        self.publisher = self.create_publisher(Twist, JOY_CMD_TOPIC, qos)

    def publish_twist(self, linear_x: float, angular_z: float) -> bool:
        """최종 스케일링된 Twist 메시지를 만들어 /joystick_cmd 토픽으로 발행합니다.

        [중요] 슬라이더 적용 + REP-103 부호 처리는 호출자(_tick_publish)에서
        이미 끝낸 '최종 값' 이 들어옵니다. 즉 이 메서드는 받은 값을 그대로
        Twist 에 담아 발행만 합니다(모놀리식과 동일한 계약).
        ...
        try:
            # ROS2 미들웨어(DDS)를 통해 전송.
            # publish() 는 thread-safe 하며 executor.spin() 과 독립적으로 동작합니다.
            # (이 호출이 Qt 메인 스레드에서 일어나도 안전한 이유 — main() 주석 참고)
    ...
    def subscriber_count(self) -> int:
        """이 퍼블리셔를 구독 중인 서브스크라이버(=Node 2 cmd_vel_relay)의 수를 반환합니다.

        2-노드 구조에서 이 값의 의미가 바뀌었습니다:
          · 모놀리식: "로봇/시뮬레이터가 cmd_vel 을 구독 중인가?"
          · 지금     : "Node 2(cmd_vel_relay)가 /joystick_cmd 에 연결되어 있는가?"
        0 이면 cmd_vel_relay 가 아직 안 떴거나, 그 구독자의 QoS 가
        BEST_EFFORT 와 비호환(예: RELIABLE)이라 연결 자체가 안 맺어진 것입니다.
...
class MainWindow(QMainWindow):
    ...
    JoystickPublisherNode(ROS2 노드)를 받아서 GUI 위젯들을 배치하고,
    QTimer 를 통해 주기적으로 /joystick_cmd 를 발행합니다.

    [Qt 스레드와 ROS spin 의 관계]
      이 클래스는 Qt 메인 스레드(GUI 스레드)에서 실행됩니다.
      ROS2 의 executor.spin() 은 별도의 백그라운드 스레드(spin_thread)에서 실행됩니다.
      이 노드는 ROS 콜백이 0개이므로 spin 스레드는 사실상 거의 할 일이 없지만,
      rclpy 컨텍스트 유지를 위해 관례적으로 spin 을 돌립니다.
      publish_twist() 는 thread-safe 하므로 Qt 스레드에서 직접 호출해도 안전합니다.
    ...
    [QTimer vs rclpy Timer]
      ...
      → 또한 이 노드는 의도적으로 rclpy 타이머를 두지 않으므로(스펙상 'ROS 콜백 0개'),
        발행 주기를 만드는 책임 자체가 QTimer 에 있습니다.
    """

    def __init__(self, node: JoystickPublisherNode):
        super().__init__()
        self.node = node  # ROS2 노드 참조 보관 (발행에 사용)
        self.setWindowTitle('J100 Joystick Publisher (Node 1 → /joystick_cmd)')
        ...
        # 참고: 이 E-STOP 은 "Node 1 이 0 을 발행" 하게 만드는 것입니다.
        #       로봇 단의 하드 비상정지(별도 안전 채널)와는 다르며,
        #       2-노드 구조에서도 Node 1 단의 1차 안전 장치로 그대로 유지합니다.
        ...
        readout = QVBoxLayout()
        readout.addWidget(QLabel('<b>/joystick_cmd (live)</b>'))
        ...
        hint = QLabel(
            ...
            "Press <b>Space</b> for emergency stop.<br>"
            "Needs <b>cmd_vel_relay</b> (Node 2) running to actually move.</i>"
        )
        ...
        #   (또한 스펙상 이 노드는 'rclpy 타이머/콜백 0개' 라서 rclpy 타이머를 둘 수 없습니다.)
        #
        # watchdog 관점 (2-노드 구조에서의 변화):
        #   모놀리식에서는 로봇 측 twist_mux 워치독(0.5 s)에 안 걸리려고 자주 보냈습니다.
        #   지금은 그 워치독 책임이 Node 2(cmd_vel_relay)로 옮겨갔지만,
        #   Node 2 역시 "최근 입력이 끊겼는지"를 보고 안전 0 을 낼지 판단하므로,
        #   이 노드는 여전히 20 Hz = 50 ms 의 끊김 없는 스트림을 내보내야 합니다.
        #   20 Hz 면 일반적인 워치독 타임아웃보다 10배가량 빠르므로 안전합니다.
        ...
        # 이전 실행에서 Node 2/로봇이 움직이고 있었던 경우를 대비합니다.
    ...
    def _on_estop(self, on: bool):
        ...
        # on=False → E-STOP 해제: 다시 조이스틱 입력을 받을 준비가 됩니다.
        self._estopped = on
    ...
    def _tick_publish(self):
        ...
        조이스틱 정규화 값 → 실제 속도(m/s, rad/s) 변환 후 /joystick_cmd 로 Twist 발행.
        [중요] 슬라이더 적용 + REP-103 부호 처리는 모놀리식과 100% 동일하게
        '여기서' 끝냅니다. Node 2 로 raw 값을 넘기는 게 아니라, Node 1 이
        '최종 스케일링된 Twist' 를 발행합니다(스펙 요구사항).
        """
        if self._estopped:
            # E-STOP 중에는 zero Twist 를 계속 발행합니다.
            # "아무것도 안 보내는" 것이 아니라 0 을 능동적으로 보내야
            # Node 2 워치독 타임아웃 없이 로봇이 정지 상태를 깔끔하게 유지합니다.
            lin, ang = 0.0, 0.0
        else:
            # ── 조이스틱 정규화 값 → SI 단위 속도 변환 (모놀리식과 동일 수식) ──
        ...
        # 계산된 선속도(lin, m/s) 와 각속도(ang, rad/s) 로 Twist 를 /joystick_cmd 에 발행.
        ...
        # health: subscriber count — 구독자 수로 Node 2 연결 상태를 진단합니다.
        # 0 이면 cmd_vel_relay(Node 2)가 안 떴거나 QoS 비호환(예: RELIABLE 구독)입니다.
        ...
        if self._estopped:
            ...
            color, text = '#f39c12', (
                f'●  no subscribers on {JOY_CMD_TOPIC}  — '
                f'is cmd_vel_relay (Node 2) running / QoS BEST_EFFORT?'
            )
        else:
            color, text = '#27ae60', (
                f'●  publishing @ {rate_hz:.0f} Hz to {JOY_CMD_TOPIC}  '
    ...
    def closeEvent(self, e):
        ...
        [종료 순서가 중요한 이유]
          ...
          2. node.publish_twist(0.0, 0.0) → ROS2 가 완전히 종료되기 전에
             zero Twist 를 /joystick_cmd 로 한 번 더 발행합니다. 이렇게 하면
             프로그램 종료 직전에 Node 2 가 마지막 이동 명령을 들고 있지 않게 됩니다.
...
def main():
    ...
    # ── 2. 노드 생성 ──────────────────────────────────────────────────
    # JoystickPublisherNode() 인스턴스화 시 내부에서
    # super().__init__('joystick_publisher') 가 호출되어 ROS2 그래프에
    # 'joystick_publisher' 노드가 등록됩니다.
    # 실행 중에 `ros2 node list` 를 치면 /joystick_publisher 가 보입니다.
    node = JoystickPublisherNode()

    # ── 3. 실행기(Executor) 설정 — 왜 '단일 스레드' 가 옳은가 ──────────
    # Executor 는 노드의 ROS 콜백(타이머/서브스크라이버/서비스 등)을
    # 어떤 스레드 모델로 실행할지 결정합니다.
    #
    # 이 노드(Node 1)는 SingleThreadedExecutor 가 '정답' 입니다. 이유:
    #   · 이 노드에는 서브스크라이버도, rclpy 타이머도, 어떤 ROS 콜백도 없습니다.
    #     즉 executor 가 처리해야 할 콜백 이벤트가 0개입니다.
    #   · 실제 발행은 Qt 의 QTimer 콜백(_tick_publish)에서 일어나고,
    #     publisher.publish() 는 thread-safe 하며 executor.spin() 의
    #     진행 여부와 '독립적' 으로 동작합니다.
    #     → 따라서 멀티스레드로 만들어 봐야 병렬화할 콜백이 없어 무의미합니다.
    #     → 단일 스레드가 더 단순하고, 동시성 버그 가능성도 없습니다.
    #
    #   ★ 대조 (교육 포인트) ★
    #     Node 2(cmd_vel_relay)는 정반대로 MultiThreadedExecutor 가 '필요' 합니다.
    #     그 노드는 (a) /joystick_cmd 구독 콜백과 (b) 워치독 타이머 콜백이
    #     동시에 진행되어야 합니다. 단일 스레드면 한 콜백이 도는 동안 다른
    #     콜백이 막혀, 입력 처리 중 워치독이 늦게 도는(또는 그 반대) 문제가
    #     생길 수 있습니다. 그래서 Node 2 는 멀티스레드 + 콜백 그룹이 필요합니다.
    #     "Pub 전용 → 단일 스레드 / 구독+워치독 동시 → 멀티 스레드" 의 대비를
    #     기억하세요.
    ...
    # (이 노드는 콜백이 0개라 spin 은 사실상 idle 이지만, rclpy 컨텍스트/그래프
    #  유지를 위해 관례적으로 spin 스레드를 둡니다.)
    #
    # daemon=True: 메인 스레드(Qt 루프)가 종료되면 이 스레드도 자동으로 종료됩니다.
    #   → 프로그램 종료 시 spin_thread 가 무한 대기로 남아 프로세스가 안 끝나는
    #     상황을 방지합니다.
    ...
    try:
        ...
        # 이렇게 하면 창 X 버튼이 아닌 다른 경로(터미널 kill 등)로 종료될 때도
        # Node 2 가 마지막 이동 명령을 들고 남지 않아 안전합니다.
        ...
        # node.destroy_node() → 노드에 연결된 ROS2 리소스(퍼블리셔 등) 해제.
